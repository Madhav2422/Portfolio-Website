    ...
    <section className='min-h-screen flex items-center pt-20 pb-16 relative overflow-hidden'>
        ...
        {[...Array(8)].map((_, i) => (
          <div
            ...
            className={`absolute rounded-full animate-float opacity-10 ${
              ['bg-cyan-500', 'bg-purple-500', 'bg-amber-500', 'bg-emerald-500'][i % 4]
            }`}
            style={{
      ...
      <div className='max-w-[1200px] mx-auto w-full px-4'>
        ...
        <div className='hidden md:flex flex-col items-center absolute bottom-10 left-1/2 transform -translate-x-1/2'>
          <div className='h-8 w-5 border-2 border-cyan-400 rounded-full relative'>
            <div className='absolute top-1 left-1/2 transform -translate-x-1/2 h-2 w-1 bg-cyan-400 rounded-full animate-bounce'></div>
          </div>
          <span className='text-cyan-300 text-sm mt-2'>Scroll to explore</span>
        </div>
      </div>
    </section>
  )
